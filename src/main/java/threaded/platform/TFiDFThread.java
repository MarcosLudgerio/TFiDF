...
public class TFiDFThread implements Runnable {
    ...
    private static final Logger LOGGER = Logger.getLogger(TFiDFThread.class.getName());

    public TFiDFThread(Map<AtomicLong, String> document) {
        TFiDFThread.documents = document.values().stream().toList();
    }
    ...
    public Map<String, Double> calculateIDF(int countDocuments) {
        lock.lock();
    ...
    public void calculateDFAndIDF() {
        ...
        Map<String, Double> doubleMap = this.calculateIDF(documentFrequency.size());
        lock.unlock();
